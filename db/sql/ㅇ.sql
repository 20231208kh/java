...
- 영화 관리
    ...
    -스틸컷은 여러 스틸컷을 등록할 수 있따
- 극장 관리
	- 관리자는 극장을 등록/수정/삭제할 수 있다
    - 극장은 지역, 이름, 상영관 정보, 주소, 총관수를 입력하고 총 좌석수는 
      등록된 상영관에 맞게 계산이 된다
    - 지역은 등록된 지역 중 하나를 선택한다
    - 상영관 정보는 극장 등록과 함께 등록하거나 극장 등록 후 등록할 수 있따
- 상영관 관리
	- 관리자는 상영관을 등록/수정/삭제할 수 있다
    - 극장 등록 시 상영관을 추가하는 경우, 상영관 번호, 총 좌석수, 
      좌석 정보를 입력하여 등록한다
    - 극장 등록 후 상영관을 추가하는 경우, 극장 번호, 상영관 번호, 
      총 좌석수, 좌석 정보를 입력하여 등록한다
	- 상영관 번호는 극장의 총 관수보다 작거나 같아야 하며,
      같은 극장에 등록된 상영관 번호는 중복될 수 없다.
	- 좌석 정보는 행의 수를 입력하고, 각 행의 열의 수를 입력한다
    - 행의 수를 입력하면 수만큼 A부터 알파뱃이 할당된다
    - 좌석 정보를 입력이 완료되면 A1부터 좌석 정보가 할당이 된다
